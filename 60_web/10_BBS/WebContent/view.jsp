	...
	<div id="bbs">
		...
		<table>
			...
			<tbody>
				...
				<tr>
					<th>첨부파일</th>
					<td><c:if test="${empty bvo.file_name }">
				첨부파일 없음
				</c:if> <c:if test="${not empty bvo.file_name }">
			...
			<tfoot>
				<tr>
					<td colspan = "2"> 
						...
						<input type ="button" value="삭제(실습)" onclick="delete_go()">
		...
		<form action="ans_del.jsp" method="post">
			...
			<%-- 삭제처리후 게시글 상세페이지 이동시(세션에 게시글 정보 없으면 --%>
