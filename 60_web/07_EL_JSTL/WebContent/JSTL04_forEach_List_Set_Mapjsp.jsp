...
<head>
<meta charset="UTF-8">
<title>JSTL forEach : Array, List, Set, Map</title>
</head>
<body>
...
<%
	...
	System.out.println("set.contains () : " +set.contains("홍길동"));
...
%>	
	...
	<%-- (실습) person에 있는 PersonVO 객체의 이름, 나이를 화면 출력 --%>
	
	<ul>
	<c:forEach var = "listbe" items="${persons }"> 
		<p>이름 : ${listbe.name }, 나이 : ${listbe.age } </p>
		</c:forEach>
	</ul>
	<hr>
	...
	<ul>
	<c:forEach var = "listbe" items="${persons }"> 
		<p>이름 : ${listbe.getName() }, 나이 : ${listbe.getAge() } </p>
		</c:forEach>
	</ul>
