...
public class Ex03_StringExam {
	...
	public static void main(String[] args) {
		/*String str = "900108-1234567" 주민 번호 
		 1. 정확히 입력된 데이터 여부 확인
		 	(전체 자리수: 14, '-' : 7번째 확인)
		...
		//1. 정확히 입력된 데이터 여부 (전체 자리수 : 14, '-' : 7번째 확인)
		if(str.length() == 14) {
		...
		if(str.substring(6, 7).equals("-")) {
			System.out.println("[정상] '-' 문자 위치 7번째 자리");
		} else {
			System.out.println("[비정상] '-' 문자 위치 7번째 자리 아님");
		}
	...
	if(month >= 1 && month <=12) {
		System.out.println("[정상] 월 데이터가 (1~12)");
	...
	//일자 데이터 확인(1~31 범위 내에 있는 지 확인)
	int date = Integer.parseInt(str.substring(4, 6));
	if(date >= 1 && date <=31 ) {
		
		System.out.println("[정상] 일 데이터가 (1~31)");
	}
