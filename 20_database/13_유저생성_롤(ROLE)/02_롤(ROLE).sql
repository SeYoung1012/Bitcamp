...
--(DBA계정 -SYSTEM) CONNECT, RESOUCE 롤에 대한 권한 확인
SELECT * FROM DBA_SYS_PRIVS
WHERE GRANTEE IN ('CONNECT', 'RESOUCE')
...
-----------------------------
--(SYSTEM) MDGUEST 에게 PROGRAMMER 롤 부여
CREATE USER "MDGUEST" IDENTIFIED BY "mdguest"
DEFAULT TABLESPACE "USERS"
TEMPORARY TABLESPACE "TEMP";
--(SYSTEM) MDGUEST 유저에게 권한 부여
...
SELECT * FROM MADANG.AAA; --권한없음 ORA-01031: insufficient privileges
--(SYSTEM) 롤(권한) 회수(취소) 
