/*제약 조건 옵션
CASCADE : 부모 테이블 (PARENT)의 제약조건을 비활성화(삭제)
        참조 하고 있는 자녀(CHILD) 테이블의 제약 조건까지 비활성화(삭제)
        
*************************************/
SELECT * FROM DEPT;
SELECT * FROM EMP01;

--부모 테이블 데이터 삭제 - 다른 테이블에서 참조하고 있으면 삭제 불가.
--ORA-02292: integrity constraint (MADANG.EMP02_DEPTNO_FK) violated - child record found
--방법 1 : 자녀테이블 참조키 모두 삭제 또는 비활성화
DELETE FROM DEPT WHERE ID = 10;
ALTER TABLE EMP01 DISABLE CONSTRAINT SYS_C007051;
ALTER TABLE EMP02 DISABLE CONSTRAINT EMP02_DEPTNO_FK;
ALTER TABLE EMP03 DISABLE CONSTRAINT EMP03_DEPTNO_FK;

ALTER TABLE EMP01 ENABLE CONSTRAINT SYS_C007051;
...
--방법 2: 서브테이블에 있는 FK 설정을 모두 삭제 후 부모테이블 삭제
...
--서브테이블의 제약조건(FK) 삭제 후 부모테이블 삭제(C_TEST_MAIN) 삭제 처리
