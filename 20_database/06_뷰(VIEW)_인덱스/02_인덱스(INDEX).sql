...
SELECT PUBLISHER, BOOKID FROM BOOK ORDER BY PUBLISHER; --F10
SELECT * FROM BOOK WHERE BOOKNAME = '축구의 이해';
-- 인덱스 생성 : 2개 컬럼 사용
-- BOOK 테이블 PUBLISHER, PRICE 컬럼을 대상으로 IX_BOOK2 인덱스 생성
CREATE INDEX IX_BOOK2 ON BOOK (PUBLISHER, PRICE);
alter index "MADANG"."IX_BOOK" UNUSABLE;
SELECT * FROM BOOK WHERE PUBLISHER = '대한미디어';
SELECT PUBLISHER,PRICE FROM BOOK WHERE PRICE > 10000;
--WHERE 절에 PUBLISHER, PRICE컬럼 동시 사용시 IX_BOOK2
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE = 7000;
SELECT * FROM BOOK WHERE  PRICE = 7000 AND PUBLISHER ='굿스포츠' ;

--- 인덱스가 있는데 사용이 안되는 경우
SELECT PUBLISHER, PRICE FROM BOOK WHERE PUBLISHER LIKE '대한%'; --인덱스 적용

SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%'; -- 인덱스 적용 못함
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어'; -- 인덱스 적용 못함

-- 원본 데이터를 가공하는 경우(인덱스 적용안됨)
SELECT * FROM BOOK WHERE SUBSTR(PUBLISHER,1,2) = '대한'; --가공되어 인덱스 적용 X
SELECT * FROM BOOK WHERE PUBLISHER = 'Pearson';
SELECT * FROM BOOK WHERE UPPER(PUBLISHER) = 'PEARSON'; -- 가공된 데이터라 인덱스 적용 X
...
SELECT NAME, ROWID FROM CUSTOMER ORDER BY NAME;
