...
ADD_MONTHS(날짜, 개월수) : 개월수만큼 월이 증가 또는 감소
LAST_DAY(날짜) : 날짜가 속한 달의 마지막 날짜 구하기
NEXT_DAY(날짜, 요일) : 요일 날짜 구하기, 날짜 다음에 오는 첫번째 요일 구하기
...
--===========================
--1 : 1일 -> 24시간, 1시간 : 1 /24  -->1시간
--1시간 : 60분 --> 1시간 /60분 --> 1분 ::: 1/24/60 --> 1분
--1분 : 60 초 --> 1분 / 60초 -> 1초 ::: 1/ 24/60/60 --> 1초
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS')
      ,TO_CHAR(SYSDATE +(1), 'YYYY-MM-DD HH24:MI:SS')
     ,TO_CHAR(SYSDATE +(1/24), 'YYYY-MM-DD HH24:MI:SS')
      ,TO_CHAR(SYSDATE +(1/24)*5, 'YYYY-MM-DD HH24:MI:SS')
FROM DUAL;

--1일 : 86400초 , 1440분 
SELECT 1*60 "1분(초)", 60 *60 "1시간(초)", 24*60*60 "1일(초)", 24*50 "1일(분)"
FROM DUAL;
