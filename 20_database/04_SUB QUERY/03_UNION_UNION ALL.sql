...
--UNION : 중복데이터를 제외하고 합해줌(중복 데이터는 1개만 표시)
...
--UNION으로 합하기 : 중복 데이터는 1개만 사용
SELECT CUSTID, NAME FROM CUSTOMER WHERE CUSTID IN(1,2,3)
UNION
SELECT CUSTID, NAME FROM CUSTOMER WHERE CUSTID IN(3,4,5);

----UNION ALL 사용 합하기 : 중복데이터 모두 사용
SELECT CUSTID, NAME FROM CUSTOMER WHERE CUSTID IN(1,2,3)
UNION ALL
SELECT CUSTID, NAME FROM CUSTOMER WHERE CUSTID IN(3,4,5)
ORDER BY NAME --ORDER BY 절은 마지막에 작성(한번만)
;
...
;
-------------
--INTERSECT : 교집합 (중복 데이터 조회) - 조인문의 동등비교 결과와 동일
SELECT CUSTID, NAME FROM CUSTOMER WHERE CUSTID IN(1,2,3)
INTERSECT 
SELECT CUSTID, NAME FROM CUSTOMER WHERE CUSTID IN(3,4,5)
;
